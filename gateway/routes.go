...
func (s *server) routes() {
	// hdl.go
	s.router.HandleFunc(s.apiRoot+"status.json", s.method(http.MethodGet,
		s.authCheck("readwrite", s.getStatus())))
	s.router.HandleFunc(s.apiRoot+"setState", s.method(http.MethodPost,
		s.inJson(s.authCheck("readwrite", s.postSetState()))))
	s.router.HandleFunc(s.apiRoot+"setRules", s.method(http.MethodPost,
		s.inJson(s.authCheck("readwrite", s.postSetRules()))))
}
